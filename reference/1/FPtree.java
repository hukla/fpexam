...
public class FPtree extends TotalSupportTree {
	 ...
	 * <TT>FPgrowthItemPrefixSubtreeNode</TT> in which to store counts and a
	 * reference to a child branch.
	 */
	...
	protected class FPtreeNode {
		 ...
		 * 
		 * @param newNode
		 *            the reference to a new node to be included in the FP-tree.
		 */
	 ...
	 * <P>
	 * A set enumeration tree in which to store itemsets together with support
	 * values.
	 */
	...
	private class FPgrowthItemPrefixSubtreeNode {
		 ...
		 * attribute identifier starting with an element in the header table
		 * (array).
		 */
	 ...
	 * Header table.
	 * <P>
	 * Array of these structures used to link into FP-tree. All FP-tree nodes
	 * with the same identifier are linked together starting from a node in a
	 * header table (made up of <TT>HeaderTasble</TT> structures). It is this
	 * "cross" linking that gives the FP-tree its most significant advantage.
	 */
	...
	/**
	 * Temporary storage for an index into an array of FP-tree nodes. </P> Used
	 * when reassigning child reference arrays.
	 */
	 ...
	 * 
	 * @param ref
	 *            the current location in the FP-tree (<TT>rootNode</TT> at
	 *            start).
	 * @param place
	 ...
	 *            the associated support value for the given itemset.
	 * @param headerRef
	 *            the link to the appropriate place in the header table.
	 */
	...
	/* ADD TO FP TREE 1 */
	/**
	 * Searches through existing branch and if itemset found updates the support
	 * count and returns true, otherwise return false.
	 * 
	 * @param ref
	 *            the current FP-tree node reference.
	 * @param place
	 *            the current index in the given itemset.
	 * @param itemSet
	 *            the given itemset.
		...
		if (ref.childRefs != null) {
			for (int index = 0; index < ref.childRefs.length; index++) {
				...
				if (itemSet[place] == ref.childRefs[index].node.itemName) {
					ref.childRefs[index].node.itemCount = ref.childRefs[index].node.itemCount
							+ support;
					numUpdates++;
	 ...
	 * <P>
	 * Adds first attribute in itemSet and then rest of sequence.
	 * 
	 * @param ref
	 *            the current FP-tree node reference.
	 * @param place
	...
	private void addToFPtree2(FPtreeNode ref, int place, short[] itemSet,
		...
		FPgrowthItemPrefixSubtreeNode newPrefixNode = new FPgrowthItemPrefixSubtreeNode(
				itemSet[place], support, ref.node);
	 ...
	 *            the current FP-tree node reference.
	 * @param backRef
	 *            the backwards link to the previous node.
	 * @param place
	 *            the current index in the given itemset.
	 * @param itemSet
	 *            the given itemset.
	...
	private void addRestOfitemSet(FPtreeNode ref,
			FPgrowthItemPrefixSubtreeNode backRef, int place, short[] itemSet,
			int support, FPgrowthHeaderTable[] headerRef) {
		...
		if (place < itemSet.length) {
			// Create new Item Prefix Subtree Node
			FPgrowthItemPrefixSubtreeNode newPrefixNode = new FPgrowthItemPrefixSubtreeNode(
					itemSet[place], support, backRef);
			// Create new FP tree node incorporating new Item Prefix Subtree
			// Node
			FPtreeNode newFPtreeNode = new FPtreeNode(newPrefixNode);
			// Add link from header table
			addRefToFPgrowthHeaderTable(itemSet[place], newPrefixNode,
					headerRef);
			ref.childRefs = reallocFPtreeChildRefs(ref.childRefs, newFPtreeNode);
			// Add into FP tree
			addRestOfitemSet(ref.childRefs[tempIndex], newPrefixNode,
	 ...
	 *            the given attribute.
	 * @param newNode
	 *            the newly created FP-tree node.
	 * @param headerRef
	 *            the reference to the header table (array).
	 */
	 ...
	 * attribute to most common single attribute). For each item. a) Count
	 * support by following node links and add to linked list of supported sets.
	 * b) Determine the "ancestor trails" connected to the nodes linked to the
	 * current item in the header table. c) Treat the list of ancestor itemSets
	 * as a new set of input data and create a new header table based on the
	 * accumulated supported counts of the single items in the ancestor itemSets
	 * d) Prune the ancestor itemSets so as to remove unsupported items. e)
	 * Repeat (1) with local header table and list of pruned ancestor itemSets
	 * as input
	 */
	...
	/**
	 * Commences process of mining the FP tree.
	 * <P>
	 * Commence with the bottom of the header table and work upwards. Working
	 * upwards from the bottom of the header table if there is a link to an FP
	 * tree node :
	 * <OL>
	 ...
	 * local header table and (iii) populate with ancestors.
	 * <LI>If new local FP tree is not empty repeat mining operation.
	 * </OL>
	 * Otherwise end.
	 * 
	 * @param tableRef
	 *            the reference to the current location in the header table
	 *            (commencing with the last item).
	 * @param itemSetSofar
	 *            the label fot the current item sets as generated to date (null
	 *            at start).
	 */
	 ...
	 * 
	 * @param nodeLink
	 *            the start link from the header table.
	 ...
	 * 
	 * @param ref
	 *            the reference to the current node in the prefix tree
	 *            containing itemsets together with support values.
	 */
	...
	private void generateAncestorCodes(FPgrowthItemPrefixSubtreeNode ref) {
		...
		while (ref != null) {
			...
			if (ancestorCode != null)
				startTempSets = new FPgrowthSupportedSets(ancestorCode,
						support, startTempSets);
			// Next ref
			ref = ref.nodeLink;
		}
	}
	 ...
	 * 
	 * @param countArray
	 *            the array of <TT>FPgrowthColumnCounts</TT> structures
	 *            describing the single item sets (in terms of labels and
	 *            associated support), contained in a linked list of
	 *            <TT>FPgrowthSupportedSets</TT> which in turn describe the
	 *            ancestor nodes in an FP-tree that preceed the nodes identified
	 *            by following a trail of links from a particular item in the
	 *            header table.
	 */
	 ...
	 *         single item sets (in terms of labels and associated support),
	 *         contained in a linked list of <TT>FPgrowthSupportedSets</TT>
	 *         which in turn describe the ancestor nodes in an FP-tree that
	 *         preceed the nodes identified by following a trail of links from a
	 *         particular item in the header table.
	 */
	...
	/**
	 * Creates a local header table comprising those item that are supported in
	 * the count array.
	 * 
	 * @param countArray
	 *            the support for the 1 item sets.
	 * @return a FPgrowth header table.
	 */

	private FPgrowthHeaderTable[] createLocalHeaderTable(
	 ...
	 * 
	 * @param countArray
	 *            the csupport for the 1 item sets.
	 * @return a FPgrpwth header table.
	 */

	private FPgrowthHeaderTable[] localHeadTabUnordered(
			FPgrowthColumnCounts[] countArray) {
		int counter = 1;
		...
		for (int index = 1; index < countArray.length; index++) {
			if (countArray[index].support >= minSupport) {
				localHeaderTable[place] = new FPgrowthHeaderTable(
						(short) countArray[index].columnNum);
				place++;
			}
		}
	 ...
	 * 
	 * @param localHeaderTable
	 *            the FPgrpwth header table to be ordered.
	 * @param countArray
	 *            the csupport for the 1 item sets.
	 */
	 ...
	 * 
	 * @param tableRef
	 *            reference to start of header table containing links to an
	 *            FP-tree produced during the FP-tree generation process.
	...
	/**
	 * Resizes the given array of FP-tree nodes so that its length is increased
	 * by one element and new element inserted.
	 * 
	 * @param oldArray
	 *            the given array of FP-tree nodes.
	 * @param newNode
	 *            the given node to be added to the FP-tree
	 ...
	 * 
	 * @param tableRef
	 *            the reference to the local header table.
	 */
	 ...
	 * 
	 * @param ref
	 *            the reference to the given branch.
	...
	private int outputItemPrefixTree(FPgrowthItemPrefixSubtreeNode ref) {
		...
		while (ref != null) {
			System.out
					.print("(" + counter + ") " + (reconvertItem(ref.itemName))
							+ ":" + ref.itemCount + " ");
			counter++;
			ref = ref.nodeLink;
		}
	 ...
	 * 
	 * @param ref
	 *            the reference to the given FP-tree branch.
	 */
	 ...
	 * 
	 * @param ref
	 *            the reference to the given FP-tree level.
	 * @param nodeID
	 *            the root string for the node ID.
	 */
	 ...
	 * 
	 * @param ref
	 *            the reference to the given node.
	 */
	...
	/**
	 * Commence the process of outputting the ancestor trail from a local header
	 * table.
	 * 
	 * @param tableRef
	 *            the reference to the local header table.
	...
	/**
	 * Commence process of determining and outputting FP-tree storage, number of
	 * updates and number of nodes.
	 */
	 ...
	 * 
	 * @param ref
	 *            the reference to the current portion of the P-tree under
	 *            consideration.
	 * @param storage
	 *            the storage requirements so far.
