...
public class TotalSupportTree extends AssocRuleMining {
	 ...
	 * T-tree when using a T-tree either as a storage mechanism, or when adding
	 * to an existing T-tree.
	 * 
	 * @param itemSet
	 *            The given itemset. Listed in numeric order (not reverse
	 *            numeric order!).
	 * @param support
	 *            The support value associated with the given itemset.
	 */
	...
	public void addToTtree(short[] itemSet, int support) {
		...
		startTtreeRef = addToTtree(startTtreeRef, numOneItemSets + 1, endIndex,
				itemSet, support);
	}
	 ...
	 * 
	 * @param linkRef
	 *            the reference to the current array in Ttree.
	 * @param size
	 *            the size of the current array in T-tree.
	 * @param endIndex
	 *            the index of the last element/attribute in the itemset, which
	 *            is also used as a level counter.
	 * @param itemSet
	 *            the given itemset.
	 * @param support
	 *            the support value associated with the given itemset.
	...
	protected TtreeNode[] addToTtree(TtreeNode[] linkRef, int size,
			int endIndex, short[] itemSet, int support) {
		...
		if (endIndex == 0) {
			linkRef[currentAttribute].support = linkRef[currentAttribute].support
					+ support;
			return (linkRef);
		}

		// Otherwise proceed down branch and return
		linkRef[currentAttribute].childRef = addToTtree(
				linkRef[currentAttribute].childRef, currentAttribute,
	 ...
	 * the T-tree (which is know to exist in the T-tree).
	 * <P>
		...
		for (int index = 0; index < combinations.length; index++) {
			...
			if (complement != null) {
				...
				if (confidenceForAR >= confidence) {
					insertRuleintoRulelist(combinations[index], complement,
							confidenceForAR);
				}
			}
		}
	...
	public void outputTtree() {
		...
		for (short index = 1; index < startTtreeRef.length; index++) {
			if (startTtreeRef[index] != null) {
				String itemSetSofar = new Short(reconvertItem(index))
						.toString();
	...
	protected int countNumFreqSets(int size, TtreeNode[] linkRef, int num) {
		...
		for (int index = 1; index < size; index++) {
			if (linkRef[index] != null) {
				if (linkRef[index].support >= minSupport)
					num = countNumFreqSets(index, linkRef[index].childRef,
							num + 1);
			}
		}
